...
const keydownHandler = (editor: JQuery<HTMLElement>, event: KeyboardEvent) => {
    const AT = '@';
    ...
    if (trimmedText.length !== selectedText.length) {
        ...
        if (range.startContainer !== range.endContainer) range.setEnd(range.startContainer, trimmedText.length);
...
function getEditorHandle(): Promise<JQuery<HTMLElement>> {
