...
const emit_debug = (...args: any[]) => { };
//const emit_debug = console.log;

function getElementPromise(...selectorChain: string[]): Promise<JQuery<HTMLElement>> {
    let intervalHandle: NodeJS.Timeout;
    let doc: JQuery<HTMLElement>;
    return new Promise((resolve, reject) => {

        const getElement = () => {
            if (!jQuery) return undefined;

            try {
                let lmnt: JQuery<HTMLElement> = (doc ??= jQuery(document.documentElement));

                const selectors = [...selectorChain];
                let selector = null;
                while (selector = selectors.shift()) {
                    lmnt = lmnt.find(selector);
                    if (!lmnt) return undefined;
                }
...
const Api = {
    ...
    headers: {
        setCsrf: (xhr: XMLHttpRequest) => xhr.setRequestHeader('x-csrf-token', Session.csrfToken),
        setXMLHttpRequest: (xhr: XMLHttpRequest) => xhr.setRequestHeader('x-requested-with', 'XMLHttpRequest'),
    },
    ...
    fetch_success: async (response: Response): Promise<{ ok: boolean, document: JQuery.Node[] }> => {
    ...
    post: (url: string, body: URLSearchParams)
        : Promise<{ ok: boolean, document: JQuery.Node[], error?: unknown }> => {
        return fetch(url, {
            method: 'POST',
            redirect: 'follow',
...
function parseBodyClasses(body: HTMLElement): {
    ...
    function processTag(isValueNumeric: boolean, value: string) {
    ...
    classes
        ...
        .forEach((match: RegExpMatchArray) => {
            ...
            switch (key) {
                ...
                case 'entity':
                    if (isValueNumeric) { entity['id'] = value; }
                    else { entity['entityType'] = value as EntityType; }
                    break;

                // kanka-type-{typeValue}
                case 'type': entity.type = value; break;

                // kanka-tag-{id} kanka-tag-{name}
                case 'tag': processTag(isValueNumeric, value); break;

                default: console.warn("What's this? 💀🎃", match); break;
            }
        });
 ...
 */
function createMatchinessComparator<TItem extends Object>
    (
        term: string,
        converter: (item: TItem) => string = item => item.toString()
    )
    ...
    const computeMatchiness = (value: string) => {
        switch (true) {
            // exact match
    ...
    return (a: TItem, b: TItem): number => {
        ...
        const relativeMatchiness = Math.sign(scoreB - scoreA) as 0 | 1 | -1;
...
const Uri = {
    ...
    buildUri: (...segments: string[]) => [Uri.rootUri, 'w', Session.campaignID, ...segments].join('/'),
